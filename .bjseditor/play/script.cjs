...
var useKtx2CompressedTextures = false;
function isUsingKtx2CompressedTextures() {
  return useKtx2CompressedTextures;
}
__name(isUsingKtx2CompressedTextures, "isUsingKtx2CompressedTextures");
function _getKtx2TextureName(name) {
  return `${name.substring(0, name.lastIndexOf("."))}.ktx2`;
}
__name(_getKtx2TextureName, "_getKtx2TextureName");
...
// node_modules/babylonjs-editor-tools/build/src/tools/particle.js
...
var SceneComponent = class {
  ...
  createSceneButton(name, index, x, y) {
    var button = import_babylonjs_gui2.Button.CreateSimpleButton(name, index);
    button.left = x;
    button.top = y;
    button.width = "180px";
    button.height = "35px";
  ...
  createTextBlock(name, index, left, top) {
    let text = new import_babylonjs_gui2.TextBlock(name, index);
    text.text = index;
    text.color = "white";
    text.fontSize = 24;
    text.left = left;
    ...
    text.horizontalAlignment = import_babylonjs_gui2.TextBlock.HORIZONTAL_ALIGNMENT_CENTER;
    text.verticalAlignment = import_babylonjs_gui2.TextBlock.VERTICAL_ALIGNMENT_CENTER;
  ...
  onStart() {
    let advancedTexture = import_babylonjs_gui2.AdvancedDynamicTexture.CreateFullscreenUI("myUI", true, this.scene);
    let button1 = this.createSceneButton(
      "button1",
      "Click Me!",
      "0px",
      "0px"
    ...
    const grid = new import_babylonjs_gui2.Grid();
    ...
    grid.addRowDefinition(50, true);
    const rect1 = new import_babylonjs_gui2.Rectangle();
    rect1.background = "#76d56e88";
    rect1.thickness = 0;
    rect1.addControl(this.heading1);
    const rect2 = new import_babylonjs_gui2.Rectangle();
    rect2.background = "#60955b88";
    rect2.thickness = 0;
    rect2.addControl(button1);
    const rect3 = new import_babylonjs_gui2.Rectangle();
    rect3.background = "#76d56e88";
    rect3.thickness = 0;
    const rect4 = new import_babylonjs_gui2.Rectangle();
    rect4.background = "#60955b88";
    rect4.thickness = 0;
    const rect5 = new import_babylonjs_gui2.Rectangle();
    rect5.background = "#76d56e88";
    rect5.thickness = 0;
    rect5.addControl(this.text1);
    const rect6 = new import_babylonjs_gui2.Rectangle();
    rect6.background = "#60955b88";
    rect6.thickness = 0;
    rect6.addControl(this.text2);
    const rect7 = new import_babylonjs_gui2.Rectangle();
    rect7.background = "#76d56e88";
    rect7.thickness = 0;
    rect7.addControl(this.text3);
    const rect8 = new import_babylonjs_gui2.Rectangle();
    ...
    this.scene.registerBeforeRender(() => {
      let mystash = this.scene.getExternalData("stash");
      try {
      ...
      ;
      try {
        this.text2.text = mystash.x;
      } catch {
      }
      try {
        this.text3.text = mystash.z;
      } catch {
      }
    });
  }
...
// src/scripts/farmer.ts
var farmer_exports = {};
__export(farmer_exports, {
  default: () => SceneComponent2
});
var import_babylonjs72 = require("babylonjs");
var SceneComponent2 = class {
  constructor(mesh) {
    this.mesh = mesh;
    //stash for messages to other scripts via externalData
    this.stash = { message: "Empty Stash" };
    this.keyDownMap = {};
    //Camera
    this.cameraMaxDistance = 900;
    this.cameraMinDistance = 600;
    this.cameraMotionRate = 0.04;
    this.cameraOffsetY = 500;
    this.stepf = new import_babylonjs72.Vector3(0, 0, 9);
    // +z
    this.stepb = new import_babylonjs72.Vector3(0, 0, -9);
    // -z
    this.stepr = new import_babylonjs72.Vector3(9, 0, 0);
    // +x
    this.stepl = new import_babylonjs72.Vector3(-9, 0, 0);
    ...
    this.platform1Aggregate = null;
    this.platform2Aggregate = null;
    this.platform3Aggregate = null;
    this.platform4Aggregate = null;
    this.cube1Aggregate = null;
    this.cube2Aggregate = null;
    this.playerAggregate = null;
    this.root = new import_babylonjs72.TransformNode("root", this.scene);
    //jumping parameters
    this.jumpHeight = 100;
    // height of jump
    this.jumpSpeed = 300;
    // speed of jump
    this.jumpTimeout = 3;
    // minimum gap between jump (seconds)
    this.jumpStart = 0;
    // start time of jump
    this.jumpEnd = 0;
    // end time of jump
    this.jumpDuration = 0;
    // duration of jump
    this.jumpUp = new import_babylonjs72.Vector3(0, 1, 0);
    // direction of jump
    ...
    this.animation1 = /* @__PURE__ */ __name(() => {
      const range = 200;
      const frameCount = 2 * this.frameRate;
      const startValue = this.platform1.position.y;
      const ySlide = new import_babylonjs72.Animation(
        "ySlide",
        "position.y",
        this.frameRate,
        import_babylonjs72.Animation.ANIMATIONTYPE_FLOAT,
        import_babylonjs72.Animation.ANIMATIONLOOPMODE_CYCLE
      );
      const keyFramesY = [];
      ...
      keyFramesY.push({
        frame: frameCount - 1,
        value: startValue + range * (this.frameRate / 2 - 1) / (this.frameRate / 2)
      });
      ySlide.setKeys(keyFramesY);
      return ySlide;
    }, "animation1");
    // functions to call on collision
    this.collideCB = /* @__PURE__ */ __name((collision) => {
      console.log(
        "collideCB",
    ...
    this.collideCB1 = /* @__PURE__ */ __name((collision) => {
      console.log(
        "collideCB1",
        collision.collider.transformNode.name,
        collision.collidedAgainst.transformNode.name
    ...
    this.forward = import_babylonjs72.Quaternion.RotationAxis(
      new import_babylonjs72.Vector3(0, 1, 0),
      0 * Math.PI / 2
    );
    this.backward = import_babylonjs72.Quaternion.RotationAxis(
      new import_babylonjs72.Vector3(0, 1, 0),
      2 * Math.PI / 2
    );
    this.left = import_babylonjs72.Quaternion.RotationAxis(
      new import_babylonjs72.Vector3(0, 1, 0),
      3 * Math.PI / 2
    );
    this.right = import_babylonjs72.Quaternion.RotationAxis(
      new import_babylonjs72.Vector3(0, 1, 0),
      1 * Math.PI / 2
    );
    ...
    this.mesh.position = import_babylonjs72.Vector3.Zero();
    this.mesh.parent = this.root;
    this.playerAggregate = new import_babylonjs72.PhysicsAggregate(
      this.root,
      import_babylonjs72.PhysicsShapeType.CAPSULE,
    ...
    this.playerAggregate.body.setMotionType(import_babylonjs72.PhysicsMotionType.DYNAMIC);
    this.playerAggregate.body.disablePreStep = false;
    this.playerAggregate.body.setMassProperties({
      inertia: import_babylonjs72.Vector3.ZeroReadOnly
    });
    ...
    this.stash.x = "-";
    this.stash.z = "-";
    this.deathAnim = this.scene.getAnimationGroupByName("Death");
    this.gun_ShootAnim = this.mesh.getScene().getAnimationGroupByName("Gun_Shoot");
    this.hitRecieveAnim = this.mesh.getScene().getAnimationGroupByName("HitRecieve");
    this.hitRecieve_2Anim = this.mesh.getScene().getAnimationGroupByName("HitRecieve_2");
    this.idleAnim = this.scene.getAnimationGroupByName("Idle");
    this.idle_GunAnim = this.mesh.getScene().getAnimationGroupByName("Idle_Gun");
    this.idle_Gun_PointingAnim = this.mesh.getScene().getAnimationGroupByName("Idle_Gun_Pointing");
    this.idle_Gun_ShootAnim = this.mesh.getScene().getAnimationGroupByName("Idle_Gun_Shoot");
    this.idle_NeutralAnim = this.mesh.getScene().getAnimationGroupByName("Idle_Neutral");
    this.idle_SwordAnim = this.mesh.getScene().getAnimationGroupByName("Idle_Sword");
    this.interactAnim = this.mesh.getScene().getAnimationGroupByName("Interact");
    this.kick_LeftAnim = this.mesh.getScene().getAnimationGroupByName("Kick_Left");
    this.kick_RightAnim = this.mesh.getScene().getAnimationGroupByName("Kick_Right");
    this.punch_LeftAnim = this.mesh.getScene().getAnimationGroupByName("Punch_Left");
    this.punch_RightAnim = this.mesh.getScene().getAnimationGroupByName("Punch_Right");
    this.rollAnim = this.scene.getAnimationGroupByName("Roll");
  ...
  getDistanceToNode(checkNode) {
    if (!this.player || !checkNode) {
      return Infinity;
    }
    const playerPos = this.player.position.clone();
    const activatorPos = checkNode.getAbsolutePosition();
    return import_babylonjs72.Vector3.Distance(playerPos, activatorPos);
  }
  onStart() {
    ...
    this.groundAggregate = new import_babylonjs72.PhysicsAggregate(
      this.ground,
      import_babylonjs72.PhysicsShapeType.BOX,
      { mass: 0, restitution: 0.2, friction: 0.7 },
      this.scene
    );
    this.groundAggregate.body.setCollisionCallbackEnabled(false);
    this.cube1Aggregate = new import_babylonjs72.PhysicsAggregate(
      this.cube1,
      import_babylonjs72.PhysicsShapeType.BOX,
    ...
    this.cube1Aggregate.body.setPrestepType(import_babylonjs72.PhysicsPrestepType.ACTION);
    this.cube1Aggregate.transformNode.animations.push(this.animation1());
    this.cube2Aggregate = new import_babylonjs72.PhysicsAggregate(
      this.cube2,
      import_babylonjs72.PhysicsShapeType.BOX,
    ...
    this.cube2Aggregate.body.setPrestepType(import_babylonjs72.PhysicsPrestepType.ACTION);
    this.platform1Aggregate = new import_babylonjs72.PhysicsAggregate(
      this.platform1,
      import_babylonjs72.PhysicsShapeType.BOX,
    ...
    this.platform1Aggregate.body.setPrestepType(import_babylonjs72.PhysicsPrestepType.ACTION);
    this.platform1Aggregate.body.setMotionType(import_babylonjs72.PhysicsMotionType.ANIMATED);
    this.platform1Aggregate.transformNode.animations.push(this.animation1());
    this.platform2Aggregate = new import_babylonjs72.PhysicsAggregate(
      this.platform2,
      import_babylonjs72.PhysicsShapeType.BOX,
      { mass: 0, restitution: 0.3, friction: 1 },
      this.scene
    ...
    this.platform3Aggregate = new import_babylonjs72.PhysicsAggregate(
      this.platform3,
      import_babylonjs72.PhysicsShapeType.BOX,
      { mass: 0, restitution: 0.3, friction: 1 },
      this.scene
    ...
    this.platform4Aggregate = new import_babylonjs72.PhysicsAggregate(
      this.platform4,
      import_babylonjs72.PhysicsShapeType.BOX,
      { mass: 0, restitution: 0.3, friction: 1 },
      this.scene
    ...
    this.platform4Aggregate.body.setMotionType(import_babylonjs72.PhysicsMotionType.ANIMATED);
    this.cube2Aggregate = new import_babylonjs72.PhysicsAggregate(
      this.cube2,
      import_babylonjs72.PhysicsShapeType.BOX,
    ...
    this.platform1Aggregate.shape.filterCollideMask = FILTER_GROUP_CUBE | FILTER_GROUP_PLAYER;
    this.platform2Aggregate.shape.filterCollideMask = FILTER_GROUP_CUBE | FILTER_GROUP_PLAYER;
    this.platform3Aggregate.shape.filterCollideMask = FILTER_GROUP_CUBE | FILTER_GROUP_PLAYER;
    this.platform4Aggregate.shape.filterCollideMask = FILTER_GROUP_CUBE | FILTER_GROUP_PLAYER;
    this.cube1Aggregate.shape.filterCollideMask = FILTER_GROUP_CUBE | FILTER_GROUP_PLATFORM | FILTER_GROUP_PLAYER;
    this.cube2Aggregate.shape.filterCollideMask = FILTER_GROUP_CUBE | FILTER_GROUP_PLATFORM | FILTER_GROUP_PLAYER;
    this.playerAggregate.shape.filterCollideMask = FILTER_GROUP_PLATFORM | FILTER_GROUP_CUBE | FILTER_GROUP_OBSTACLE;
    this.groundAggregate.shape.filterCollideMask = FILTER_GROUP_CUBE | FILTER_GROUP_PLAYER | FILTER_GROUP_OBSTACLE;
    ...
    this.scene.actionManager = new import_babylonjs72.ActionManager(this.scene);
    this.scene.actionManager.registerAction(
      new import_babylonjs72.ExecuteCodeAction(
        {
          trigger: import_babylonjs72.ActionManager.OnKeyDownTrigger
        },
        (evt) => {
    ...
    this.scene.actionManager.registerAction(
      new import_babylonjs72.ExecuteCodeAction(
        {
          trigger: import_babylonjs72.ActionManager.OnKeyUpTrigger
        },
        (evt) => {
    ...
    this.scene.onBeforeRenderObservable.add(() => {
      var cameraDirection = this.camera.getDirection(new import_babylonjs72.Vector3(0, 0, 1));
      cameraDirection.y = 0;
      cameraDirection.normalize();
      this.camera.setTarget(
        import_babylonjs72.Vector3.Lerp(
      ...
      var dist = import_babylonjs72.Vector3.Distance(
        this.camera.position,
        this.player.position.clone()
      );
      const amount = (Math.min(dist - this.cameraMinDistance, 0) + Math.max(dist - this.cameraMaxDistance, 0)) * this.cameraMotionRate;
      cameraDirection.scaleAndAddToRef(amount, this.camera.position);
      this.camera.position.y += (this.player.position.y + this.cameraOffsetY - this.camera.position.y) * this.cameraMotionRate;
      const newY = 40 + (Math.cos(this.time * 1.3) + 1) * 100;
      this.platform4Aggregate.body.setTargetTransform(
        new import_babylonjs72.Vector3(
          this.platform4.position.x,
          newY,
          this.platform4.position.z
        ),
        this.platform4.rotationQuaternion || import_babylonjs72.Quaternion.Identity()
      );
      this.time += this.scene.getEngine().getDeltaTime() / 1e3;
      this.stash.message = this.getDistanceToNode(this.activator).toString();
    });
  }
  onUpdate() {
    var keydown = false;
    if (this.keyDownMap["j"] || this.keyDownMap["ArrowLeft"]) {
      ...
      this.playerAggregate.body.setLinearVelocity(
        new import_babylonjs72.Vector3(-90 * this.ratio, 0, 0)
      );
    ...
    if (this.keyDownMap["k"] || this.keyDownMap["ArrowDown"]) {
      ...
      this.walkAnim.start(true);
      this.playerAggregate.body.setLinearVelocity(
        new import_babylonjs72.Vector3(0, 0, -90 * this.ratio)
      );
    ...
    if (this.keyDownMap[" "] && this.jumpEnabled) {
      if (!this.isJumping) {
        ...
        this.playerAggregate.body.setLinearVelocity(
          this.playerAggregate.body.getLinearVelocity().clone().addInPlace(new import_babylonjs72.Vector3(0, this.jumpSpeed * this.ratio, 0))
          // times this.ratio)
        );
      } else {
        ...
        if (this.playerAggregate.body.transformNode.position.y >= this.jumpEnd) {
          console.log("jump end");
          this.isJumping = false;
          this.jumpEnabled = false;
          setTimeout(() => {
            this.jumpEnabled = true;
          }, this.jumpTimeout * 1e3);
        }
      }
      keydown = true;
    }
    if (!keydown) {
      if (this.walkAnim?.isPlaying) {
        this.walkAnim.stop();
        this.idleAnim.start(true);
        this.playerAggregate.body.setLinearVelocity(new import_babylonjs72.Vector3(0, 0, 0));
      }
    }
    if (this.keyDownMap["x"] || this.keyDownMap["X"]) {
      ...
      this.deathAnim.goToFrame(this.deathAnim.to);
    }
    if ((this.keyDownMap["t"] || this.keyDownMap["T"]) && !this.gun_ShootAnim.isPlaying) {
      this.idleAnim.stop();
      this.walkAnim.stop();
      ...
      this.gun_ShootAnim.goToFrame(this.gun_ShootAnim.to);
    }
  }
};
...
__export(box_exports, {
  default: () => SceneComponent3
});
var SceneComponent3 = class {
  constructor(mesh) {
    this.mesh = mesh;
    this.mesh = mesh;
...
// src/scripts.ts
var scriptsMap = {
  "scripts/gui.ts": gui_exports,
  "scripts/farmer.ts": farmer_exports,
